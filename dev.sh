...
function dknpminstall {
    CD=$(pwd)
    cd $PROJ_BASE
    docker run -it --rm -v $(pwd):/app -w /app/frontend -e NODE_ENV=development apple npm install
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dkrun_prod {
    docker stop apple
    docker rm apple
    docker run --name apple -d --env-file /home/ubuntu/apple.env \
        -p 3000:3000 -p 8000:8000 \
        -v /home/ubuntu/dkdata/apple:/dkdata \
        apple start_web.sh
}

function deploy_prod {
  rsync -av --exclude frontend/node_modules --exclude frontend/.nuxt * ubuntu@$HOST_PROD:./apple/
...
echo_green "Welcome to apple's dev env"
