...
```bash
dk bash  # starts bash inside "apple" container
...
* `dkup` uses docker-compose to start 3 containers: postgres, nginx, and apple.
...
* Once `dkup` is running, `dk <command>` will run `<command>` inside the `apple` container. So `dk bash` will get you "logged in" as root inside that container. Once inside, you need to run Django's `manage.py` commands to initialize the database properly.
* The apple container runs 3 services:
 * django on port 8000
...
```bash
mkvirtualenv apple -p python3  # creates a python3 virtualenv
...
Install docker and nginx. Create an empty postgres database apple owned by a user apple.

On your remote machine, create a file ~/apple.env:

```
DJANGO_DB_PASSWORD=<apple's password>
DJANGO_DB_HOST=<database_ip>
DJANGO_DB_NAME=apple
DJANGO_DB_USER=apple
DJANGO_DEBUG=0
```
...
server {
    ...
    location /static {
        alias /home/ubuntu/dkdata/apple/static;
...
(Replace "apple.example.com" with your production domain)
